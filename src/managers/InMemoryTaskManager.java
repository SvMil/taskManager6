...
public class InMemoryTaskManager implements TaskManager {
    ...
    protected List<Task> tasksListSortByDate = new ArrayList<>();
    ...
    public void createTask(Task newTask) {
        ...
        try {
            if(newTask.getStartTime()!=null){
                checkCrossDate(newTask);}
        }catch (IntersectionTasksException exception) {
    ...
    public void newCreateTask(Task newTask) {
        newTask.setId(generateId());
        if (taskList == null) {
            taskList = new HashMap<>();
        }
    ...
    protected void epicStatusControl(Integer epicId) {
        ...
        epicStartTimeControl(epic);
        epicEndTimeControl(epic);
    }
    ...
    public void checkCrossDate(Task task) {
        LocalDateTime startNewTask = task.getStartTime();
        LocalDateTime endNewTask = task.getEndTime();
        for(Task sortTask : tasksListSortByDate){
            if (!(task.getStartTime().isAfter(sortTask.getEndTime())||task.getEndTime().isBefore(sortTask.getStartTime()))) {
                throw new IntersectionTasksException("Ошибка добавления задачи, в системе уже есть задача с пересекающимся периодом выполнения ",task,sortTask);
            }
        }
    }

    protected List<Task> getEpicSubTaskList(Integer epicId) {
    Epic epic = epicList.get(epicId);
    ArrayList<Integer> tasksId = epic.getSubTasksList();
...
public void epicStartTimeControl(Epic epic) {
    List<Task> subTasks = getEpicSubTaskList(epic.getId());
    if (!subTasks.isEmpty()) {
        List<Task> result = subTasks
                .stream()
                .filter(tasks -> tasks.getStartTime() != null)
                .collect(Collectors.toList());

        if(result.size()!=0) {
        result.sort((Task e1, Task e2) -> e1.getStartTime().compareTo(e2.getStartTime()));
            epic.setStartTime(result.get(0).getStartTime());
        }
    }
}

public void epicEndTimeControl(Epic epic) {
    List<Task> subTasks = getEpicSubTaskList(epic.getId());
    if (!subTasks.isEmpty()) {
        List<Task> result = subTasks
                .stream()
        ...
        if (result.size() != 0) {
            result.sort((Task e1, Task e2) -> e1.getStartTime().plus(e1.getTaskDuration()).compareTo(e2.getStartTime().plus(e2.getTaskDuration())));
            epic.setEndTime(result.get(result.size() - 1).getStartTime().plus(result.get(0).getTaskDuration()));
        }
    }
}

public List<Task>  getPrioritizedTasks(){
    return tasksListSortByDate;
}
public void setPrioritizedTasks() {
    ...
    */
    ...
    setTasksListSortByDate(allTasksList);

}

public void setTasksListSortByDate(List<Task> taskList) {
    this.tasksListSortByDate = taskList;
}

    public List<Task> getTasksListSortByDate() {
        return tasksListSortByDate;
    }
